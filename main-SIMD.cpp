...
fstream BasisFile("消元行.txt", ios::in | ios::out);
...
void reset() {
	...
	BasisFile.open("消元行.txt", ios::in | ios::out);
	iToBasis.clear();
	iToFirst.clear();
	ans.clear();
...
void generateBasisFile(const string& filename, int numRows, int maxSize, int maxValue) {
    ofstream outFile(filename);
    srand(static_cast<unsigned>(time(0)));
    for (int i = 0; i < numRows; i++) {
        int numElements = rand() % maxSize + 1;
        vector<int> positions;
        for (int j = 0; j < numElements; j++) {
            positions.push_back(rand() % maxValue);
        }
...
void generateRowsFile(const string& filename, int numRows, int maxSize, int maxValue) {
    ofstream outFile(filename);
...
int main() {
	...
	// 生成消元行和被消元行文件
    generateBasisFile("消元行.txt", 100, 50, 1000);
    generateRowsFile("被消元行.txt", 200, 50, 1000);

    cout << "消元行和被消元行文件已生成。" << endl;
